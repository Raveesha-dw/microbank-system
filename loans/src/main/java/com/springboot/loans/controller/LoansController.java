...
public class LoansController {
    ...
    @Autowired
    private LoansContactInfoDto contactInfo;
    ...
    @GetMapping("/fetch")
    public ResponseEntity<LoansDto> fetchLoanDetails(@RequestHeader("microbank-correlation-id") String correlationId,
                                                    @RequestParam
                                                     @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                     String mobileNumber) {
    ...
    @Operation(
            ...
            description = "Get Contact Information of loans microservice"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
